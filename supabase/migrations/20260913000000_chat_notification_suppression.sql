-- Chat notification behaviour (2026-09-13):
--   1. Don't create a notification for a recipient who is actively viewing the
--      conversation (heartbeat written by the client, valid for 2 minutes).
--   2. At most ONE unread message notification per conversation — a burst of
--      unseen messages collapses to the latest one.
--   3. Honour the existing `conversation_participants.muted` flag.
--
-- The client marks a conversation "viewing" while its chat pane is mounted
-- (refreshed every 60s) and marks that conversation's notifications read when
-- it opens, so the bell never lights up for the chat you are already reading.

alter table public.conversation_participants
  add column if not exists viewing_at timestamptz;

create index if not exists idx_conv_participants_viewing
  on public.conversation_participants(conversation_id, user_id, viewing_at);

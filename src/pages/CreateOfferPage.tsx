...
const TOKEN_DECIMALS: Record<string, number> = {
  // Mainnet deployment pins USDC (6 decimals) as the escrow token. The map
  // still covers the other symbols the offer form allows.
  USDC: 6,
  USDT: 6,
  DAI: 18,
  ETH: 18,
...
interface OfferForm {
  ...
  token: string
  fiatCurrency: string
  // Numeric fields are kept as STRINGS so the controlled inputs can be empty
  // while editing (clearing "100" to type a new value). Converting to a number
  // on every keystroke would coerce `''` to `0` and make the field impossible
  // to clear/edit. Parsed with Number() at validation/submit time.
  price: string
  minAmount: string
  maxAmount: string
...
export function CreateOfferPage() {
  ...
  // The factory pins a single ERC-20 (USDC on mainnet) as the escrow token —
  // offers in any other token can never be escrowed.
  const tokens = ['USDC']

...
> Combined reference for opencode sessions: design system + working agreements.
> Source of truth for the design rules lives in [`claude.md`](claude.md) (and the
> historical alias `design.md` — same content). Keep the two in sync if either
> changes. Anything below that overlaps `claude.md` mirrors it verbatim on
> purpose; do not let them drift.
